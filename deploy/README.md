...
The systemd unit loads `XAI_API_KEY` from `~/ai-datacenters/.env`:

```bash
cd ~/ai-datacenters
umask 077                              # ensure 600 permissions on the new file
printf 'XAI_API_KEY=xai-REPLACE_ME\n' > .env
ls -l .env                             # confirm -rw-------
```
...
```bash
$EDITOR ~/ai-datacenters/.env
systemctl --user restart ai-datacenters
```
No `daemon-reload` needed — only `.env` changed, not the unit file.
